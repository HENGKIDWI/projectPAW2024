...
{
  global $conn;
  $query = 'SELECT * FROM pengumpulan_tugas WHERE "id_siswa = "' . $conn->real_escape_string($siswa) . '" AND tugas_id = "' . $conn->real_escape_string($tugas) . '"';
  return mysqli_query($conn, $query);
}
            ...
            <table class="table-auto w-full text-left border-collapse border border-gray-300">
              ...
              <tbody>
                ...
                while ($row = mysqli_fetch_assoc($daftar_murid)) {
                  ...
                  // Menampilkan hasil tugas dari siswa
                  $file_tugas = getFile( $row['id_siswa'], $selected_tugas);
                  $task_file = mysqli_fetch_assoc($file_tugas);
                  if ($task_file && !empty($task_file['file_tugas'])) {
                    // Generate a temporary file to download
                    echo "<a href='download_file.php?kelas=" . urlencode($selected_kelas) . "&siswa=" . urlencode($row['nis']) . "&tugas=" . urlencode($selected_tugas) . "' class='text-blue-600 hover:underline'>Unduh Tugas</a>";
                  } else {
                    echo "<span class='text-gray-500'>Tidak ada tugas</span>";
                  }
                  echo "</td>";
                  echo "<td class='px-4 py-2 border border-gray-300'>";
                  echo "<input type='number' name='nilai[" . $row['id_siswa'] . "]' min='0' max='100' class='w-20 p-1 border border-gray-300 rounded' required>";
                  echo "</td>";
                  echo "</tr>";
                }
                ?>
